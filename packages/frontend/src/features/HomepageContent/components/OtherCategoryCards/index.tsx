...
export function OtherCategoryCards() {
    ...
    const categoriesToDisplay = useMemo(() => {
        return categoryTree.flatMap((category) => (category.slug !== "coffee" ? category : []));
    }, [categoryTree]);
            ...
            .map((category, i) => {
                ...
                return (
                    <Carousel.Slide
                        ...
                        key={slug}
                    >
                        <div className={styles["other-category-container"]}>
                            <div
                                className={styles["other-category-image-container"]}
                                style={{ visibility: displaySkeletons ? "hidden" : "initial" }}
                            >
                                <Image className={styles["other-category-image"]} src="" alt="" />
                            </div>

                            <div
                                className={styles["other-category-information-container"]}
                                style={{ visibility: displaySkeletons ? "hidden" : "initial" }}
                            >
                                <p className={styles["other-category-name"]}>{name}</p>

                                <p className={styles["other-category-description"]}>
                                    {description}
                                </p>
                            </div>
                            ...
                            <button
                                type="button"
                                role="link"
                                onClick={() => navigate({ pathname: `/c/${slug}` })}
                                disabled={displaySkeletons}
                                className={styles["other-category-link"]}
                            >
                                Shop
                            </button>
                        </div>
    ...
    return (
        <section className={styles["other-category-cards"]}>
            <div className={styles["other-category-cards-width-controller"]}>
                <div className={styles["top-row-container"]}>
                    <div className={styles["title-container"]}>
                        <h2 className={styles["other-category-cards-title"]}>
                            We {`don't`} only sell coffee...
                        </h2>
                        <p className={styles["other-category-cards-subcopy"]}>
                            Try some of our exceptional tea blends, or explore the rest of our
                            range.
                        </p>
                    </div>
                </div>
