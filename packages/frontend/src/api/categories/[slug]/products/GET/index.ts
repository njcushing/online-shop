...
type RequestParams = paths["/api/categories/{slug}/products"]["get"]["parameters"];
...
export const getCategoryBySlugProducts: HTTPMethodTypes.GET<RequestParams, ResponseBody> = async (
    data,
) => {
    const { params } = data;
    ...
    if (!slug) {
        return {
            success: false as const,
            status: 400,
            message: "Could not make request: no category slug provided for endpoint path.",
            error: undefined,
        };
    }
    ...
    if (!(typeof page === "number" && Number.isInteger(page) && page > 0)) {
        return {
            success: false as const,
            status: 400,
            message: "Could not make request: no page number provided for product pagination.",
            error: undefined,
        };
    }
    ...
    const result = await fetcher<ResponseBody>(
        `${apiUrl}/categories/${slug}/products?${urlParams}`,
        {
